...
def get_video_files_recursively(folder_path):
    ...
    try:
        ...
        print(f"Found {len(video_files)} local video files in {folder_path} and subdirectories.")
        return video_files
    except Exception as e:
...
class LocalVideoDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Local Video Options")
        ...
        self.layout.addWidget(self.enable_local_videos)
...
class TileAnimator(QObject):
    ...
    def start_random_timer(self):
        ...
        # Dramatically longer intervals - 90-180 seconds (1.5-3 minutes)
        possible_intervals_ms = [90000, 120000, 150000, 180000]
    ...
    def choose_next_transition(self):
        ...
        # Favor more dramatic layout changes that are visible for longer periods
        transition_weights = {
            'swap': 0.1,         # 10% chance - simple tile swap (reduced)
            'resize': 0.6,       # 60% chance - reorganize tiles in new layout (increased further)
            'full_screen': 0.25, # 25% chance - full-screen takeover (increased for dramatic effect)
            'refresh': 0.05      # 5% chance - refresh videos (reduced)
        }
    ...
    def start_animation(self, tile, start_geom, end_rect):
        ...
        # Dramatically increase duration for extremely slow animation
        animation.setDuration(8000)  # 8 seconds for very slow, contemplative transitions
        ...
        # Slower fade effect for more graceful transitions
        tile.setStyleSheet("background-color: rgba(0, 0, 0, 0.85);")  # Very slight fade
        QTimer.singleShot(4000, lambda: tile.setStyleSheet("background-color: black;"))
...
class VideoWall(QMainWindow):
    ...
    MAX_ACTIVE_PLAYERS = 14  # Increased from 7 to 14 to allow more streams across three screens

    def __init__(self, m3u8_links, local_videos, screen, parent=None):
        super().__init__(parent)
        self.screen = screen
        self.setWindowTitle(f"Video Wall 3x3 {screen.name()} - Local Video Fallback (v1.4.2)")
        ...
        # Timing adjusted to be much slower for a more relaxed pace
        QTimer.singleShot(500, self.assign_initial_videos)
        QTimer.singleShot(15000, lambda: self.force_initial_transition())

        self.check_timer = QTimer(self)
        self.check_timer.timeout.connect(self.check_and_refresh_streams)
        self.check_timer.start(60000)  # 60 seconds - check streams once per minute
        
        self.black_tile_timer = QTimer(self)
        self.black_tile_timer.timeout.connect(self.aggressive_black_tile_check)
        self.black_tile_timer.start(15000)  # 15 seconds for black tile detection
        
        # Add resource monitoring
        self.resource_check_timer = QTimer(self)
        self.resource_check_timer.timeout.connect(self.check_resource_usage)
        self.resource_check_timer.start(60000)  # 60 seconds for resource usage checks
    ...
    def force_initial_transition(self):
        ...
        print(f"Forcing initial transition on {self.screen.name()} to start dynamic layouts")
        if self.animator:
    ...
    def refresh_all_tile_videos(self):
        ...
        count = min(len(visible_tiles), self.MAX_ACTIVE_PLAYERS)
        streams_to_assign = self.get_next_available_streams(count)
        ...
        for i, tile in enumerate(self.tiles):
            ...
            if streams_to_assign and stream_idx < len(streams_to_assign):
                ...
                if self.local_videos:
                    QTimer.singleShot(10000, lambda idx=i: self.check_loading_and_fallback(idx))
        ...
        QTimer.singleShot(30000, self.ensure_some_streams_showing)
    ...
    def ensure_some_streams_showing(self):
        ...
        # Calculate target - we want at least 80% streams if possible with higher MAX_ACTIVE_PLAYERS
        target_stream_count = int(total_visible * 0.8)
        ...
        if stream_count < target_stream_count and total_visible > 0 and self.all_m3u8_links:
            ...
            QTimer.singleShot(45000, self.ensure_some_streams_showing)
    ...
    def check_loading_and_fallback(self, tile_index):
        ...
        if status in [QMediaPlayer.NoMedia, QMediaPlayer.InvalidMedia]:
            ...
            QTimer.singleShot(10000, lambda idx=tile_index: self.check_loading_and_fallback(idx))
            ...
            if self.retry_attempts[tile_index] > 2:
                ...
                QTimer.singleShot(5000, lambda idx=tile_index: self.check_loading_and_fallback(idx))
    ...
    def force_stream_on_tile(self, tile_index):
        ...
        # Continue to monitor this stream - first check more quickly
        QTimer.singleShot(5000, lambda idx=tile_index: self.monitor_forced_stream(idx))
    ...
    def monitor_forced_stream(self, tile_index):
        ...
        if status in [QMediaPlayer.NoMedia, QMediaPlayer.InvalidMedia]:
            ...
            if self.retry_attempts[tile_index] > 3:
                ...
                QTimer.singleShot(10000, lambda idx=tile_index: self.monitor_forced_stream(idx))
            ...
            if self.retry_attempts[tile_index] > 3:
                ...
                QTimer.singleShot(8000, lambda idx=tile_index: self.monitor_forced_stream(idx))
            ...
            if state != QMediaPlayer.PlayingState:
                ...
                QTimer.singleShot(15000, lambda idx=tile_index: self.monitor_forced_stream(idx))
        else:
    ...
    def resume_visible_players(self):
        ...
        for i, tile in enumerate(self.tiles):
            ...
            if player and player.state() == QMediaPlayer.PausedState:
                player.play()
                active_count += 1
                print(f"Resumed player for Tile {tile.tile_id}")
    ...
    def handle_media_status(self, status, player, tile, tile_index):
        ...
        if status == QMediaPlayer.LoadingMedia:
            ...
            if self.local_videos and not is_local:
                ...
                QTimer.singleShot(500, lambda idx=tile_index: self.retry_tile_stream(idx))
        elif status == QMediaPlayer.StalledMedia:
            ...
            if self.local_videos and not is_local:
                QTimer.singleShot(2000, lambda idx=tile_index: self.force_local_video_fallback(idx))
            else:
                QTimer.singleShot(5000, lambda idx=tile_index: self.retry_tile_stream(idx))
        elif status == QMediaPlayer.EndOfMedia:
    ...
    def handle_player_error(self, error, player, tile, tile_index):
        ...
        if is_local:
            ...
            if self.local_videos and len(self.local_videos) > 1:
                QTimer.singleShot(500, lambda idx=tile_index: self.force_local_video_fallback(idx))
            else:
                QTimer.singleShot(500, lambda idx=tile_index: self.retry_tile_stream(idx))
        else:
            current_url_short = current_url.split('/')[-1][:25] if current_url != "N/A" else "N/A"
            ...
            if self.local_videos:
                QTimer.singleShot(300, lambda idx=tile_index: self.force_local_video_fallback(idx))
            else:
                QTimer.singleShot(500, lambda idx=tile_index: self.retry_tile_stream(idx))
    ...
    def keyPressEvent(self, event):
        ...
        if key == Qt.Key_Escape or (key == Qt.Key_Q and modifiers == Qt.ControlModifier):
            print("Ctrl+Q or Escape pressed, closing all windows...")
...
def test_stream(url, callback):
    ...
    def check_status():
        ...
        if status in [QMediaPlayer.LoadedMedia, QMediaPlayer.BufferedMedia]:
            ...
            print(f"  FAIL: Stream invalid: {url}")
            callback(url, False) 
            test_player.stop()
        else:
...
if __name__ == "__main__":
    ...
    if file_dialog.exec_():
        ...
        if selected_files: 
            ...
            # Test a few streams to see if they're working
            if m3u8_links:
                msg = QMessageBox()
    ...
    # Now show the local video dialog after getting M3U8 file
    local_video_dialog = LocalVideoDialog()
    if local_video_dialog.exec_() != QDialog.Accepted:
        print("Dialog canceled. Exiting.")
        sys.exit(0)
        
    dialog_results = local_video_dialog.get_results()
